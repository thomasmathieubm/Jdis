...
//  print_help : affiche sur la sortie standard l'aide du programme.
void print_help(const char *execname);

//  print_usage : affiche sur la sortie standard un message court sur
//    l'utilisation du programme.
void print_usage(const char *execname);

//  print_version : affiche sur la sortie standard la version du programme.
    ...
    while (c != EOF) {
      ...
      if (isspace(c) || c == EOF || (p_opt && ispunct(c))
        ...
        if (i_opt > 0 && (int) i == i_opt) {
          if (!isspace(c) && (p_opt && !ispunct(c)) && c != EOF) {
